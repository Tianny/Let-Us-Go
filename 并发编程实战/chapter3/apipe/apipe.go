...
func runCmdWithPipe() {
	fmt.Println("Run command `ps axu | grep apipe`: ")
...
func runCmd() {
	...
	if err != nil {
		fmt.Printf("Error: Could not obtain the stdout pip for command")
		return
	}
	defer func() {
		_ = stdout0.Close()
	...
	if !useBufferedIO {
		...
		if err != nil {
			fmt.Printf("Error: Couldn't read data from pipe: %s", err)
			return
		}
		fmt.Printf("%s\n", string(output0))
	}
